...
var Collapsible = React7.forwardRef(
  (props, forwardedRef) => {
    const {
      __scopeCollapsible,
      open: openProp,
...
var CollapsibleTrigger = React7.forwardRef(
  (props, forwardedRef) => {
...
var CollapsibleContent = React7.forwardRef(
  (props, forwardedRef) => {
...
var CollapsibleContentImpl = React7.forwardRef((props, forwardedRef) => {
  const { __scopeCollapsible, present, children, ...contentProps } = props;
  const context = useCollapsibleContext(CONTENT_NAME, __scopeCollapsible);
  const [isPresent, setIsPresent] = React7.useState(present);
  const ref = React7.useRef(null);
  const composedRefs = useComposedRefs(forwardedRef, ref);
  const heightRef = React7.useRef(0);
  const height = heightRef.current;
  const widthRef = React7.useRef(0);
  const width = widthRef.current;
  const isOpen = context.open || isPresent;
  const isMountAnimationPreventedRef = React7.useRef(isOpen);
  const originalStylesRef = React7.useRef(void 0);
  React7.useEffect(() => {
